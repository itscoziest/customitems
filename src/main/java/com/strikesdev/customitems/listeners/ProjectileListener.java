...
public class ProjectileListener implements Listener {
                ...
                if (currentTick % 20 == 0) {
                    for (Entity nearbyEntity : breathCenter.getWorld().getNearbyEntities(breathCenter, effectRadius, effectRadius, effectRadius)) {
                        if (nearbyEntity instanceof Player && !nearbyEntity.equals(caster)) {
                            ...
                            double newHealth = Math.max(0, currentHealth - damageToApply);
    ...
    public void onProjectileHit(ProjectileHitEvent event) {
        ...
        // FIX: Merged all Snowball logic here
        if (projectile instanceof Snowball) {
            ...
            // Swap Ball (Now a snowball instead of EnderPearl)
    ...
    // FIX: Changed signature to accept Projectile, and logic to handle Snowball
    private void handleSwapBallHit(ProjectileHitEvent event) {
        // Changed from EnderPearl to Snowball
        Snowball pearl = (Snowball) event.getEntity();

        // Note: setCancelled doesn't stop snowball hit, but we don't need it since snowballs don't teleport
        pearl.remove();
